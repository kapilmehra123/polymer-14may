...
//verify the trial expire popup for the gmail signup
describe("Verify the trial expire popup for the gmail signup", async function () {
    let getdata;
    ...
    it("Search the user and click on quick action", async function () {
        ...
        await actionWithRetry(driver, async function (element) {
            await element.sendKeys(execution[0].verify_trial_expired_popup[0].verification[0].gmail)
        }, sendUser, 3, 10000)
        await driver.sleep(shortSleepDuration);
    ...
    //Log out the admin
    it("Log out the admin", async function () {
        log('Log out the admin')
        await driver.sleep(shortSleepDuration);
    ...
    it("Login the gmail account", async function () {
        log('Login the gmail account')
        await driver.sleep(shortSleepDuration);
        const waitforname = By.xpath("//span[text()='Sign in with Google']");
        await actionWithRetry(driver, async function (element) {}, waitforname, 4, 1000);
        const signingoogleElementLocator = By.xpath("//span[text()='Sign in with Google']");
        await actionWithRetry(driver, async function (element) {
            await element.click();
    ...
    it("Verify Congratulations for upgrading! You are now using pop up", async function () {
