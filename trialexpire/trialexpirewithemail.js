...
//verify the trial expire popup
describe("Verify the trial expire popup", async function () {
    let getdata;
    ...
    it("log out the admin account", async function () {
        log('log out the admin account')
        await driver.sleep(shortSleepDuration);
    ...
    it("Login the user account", async function () {
        log('login the user account')
        //send the email
        const emailElementLocator = By.xpath("//input[@id='email']");
        await actionWithRetry(driver, async function (element) {
            await element.sendKeys(email);
